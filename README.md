...
| Phase 1 rolling-origin evaluation | ✅ **Stage D merged (PR #41)** — `src/backtesting.py` (rolling-origin folds: expanding/sliding × overlapping/non-overlapping) + `src/baselines.py` (last-value, seasonal naive) + `src/metrics.py` (MAE/RMSE/MASE) |
| Phase 1 data fingerprinting | ✅ **Merged (PR #42)** — `src/fingerprinting.py` (deterministic SHA-256 over canonical data, NaN/inf normalised) wired into the adapter's `RunMetadata.data_fingerprint` |
| Phase 1 features | 🔜 In progress — Stages C/D + fingerprinting merged; remaining slices (exports/visualisation, page integration) |
...
src/
    ...
    fingerprinting.py   # Phase 1 deterministic data fingerprinting (Slice 4)
    forecasting/
...
tests/
    ...
    test_fingerprinting.py       # Phase 1 fingerprinting tests (Slice 4)
...
| E | Phase 1 start | 🔜 In progress — **Stages C + D + fingerprinting merged (PRs #40/#41/#42)**; remaining Phase 1 slices pending | 7 |
...
> resumed: Stage C (typed ingestion/validation, PR #40), Stage D (rolling-origin
> backtesting + baselines + metrics, PR #41) and Slice 4 fingerprinting (PR #42)
> are merged.
...
- **Phase 1 ingestion/validation (Stage C)**: Core ingestion module merged (PR #13);
...
- **Phase 1 rolling-origin evaluation (Stage D)**: **merged (PR #41)** — `src/backtesting.py`
  ...
  eligibility contract), `src/metrics.py` (MAE/RMSE/MASE with MASE scaling).
- **Phase 1 data fingerprinting (Slice 4)**: **merged (PR #42)** — `src/fingerprinting.py`
  computes a deterministic SHA-256 over canonical task data (sorted-key JSON, NaN/inf
  normalised as canonical markers); the `Chronos2Adapter` now populates
  `RunMetadata.data_fingerprint`. All Phase 1 slices so far are module + tests; Streamlit
  page integration of the Phase 1 pipeline is a later step.
